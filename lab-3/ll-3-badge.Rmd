...
-   The best tuned model was around 88.6% accurate and the worst was 86.7%, so about a 2% point boost in performance. Was it worth the investment? Maybe. Depends on how it will be used. 
...
-   Kappa (.75) gives a sense of the agreement between the model and truth, accounting for chance. It does pretty well here. Sensitivity (0.89) and specificity (.86) are measures of the type of errors it gives. AUC (.932) computes the sensitivity-specificity tradeoff, and it does really well here.  
...
1.  Provide an APA citation for your selected study.
    ...
    -   Zhang, C., Huang, Y., Wang, J., Lu, D., Fang, W., Stamper, J., Fancsali, S., Holstein, K., Aleven, V.: Early detection of wheel spinning: Comparison across tutors, models, features, and operationalizations. EDM, 2019
...
2.  What research questions were the authors of this study trying to address and why did they consider these questions important?
    ...
    -   They were trying to detect “Wheel spinning”, which is the phenomenon in which a student fails to master a Knowledge Component (KC), despite significant practice. This is important because you don't want students spending unneccessary time in a task. 

3.  What were the results of these analyses?

    -   They compared several models aimed at early detection of wheel spinning, including Random Forest and Logistic Regression. The two most accurate models were Random Forest and Multi-Layer Perceptron. Random Forest had Precision/recall of 77.2% and 63.5% on one outcome, and precision/recall of 90.8% and 81.4% on another. 
...
```{r random-forest approach, eval=FALSE, include=FALSE}
library(tidyverse)
library(tidymodels)
library(vip) # a new package we're adding for variable importance measures
setwd("~/Documents/GitHub/LASER/machine-learning/lab-3/")
...
kfcv <- vfold_cv(data_train,v = 3) # again, we will use resampling
...
-   The best fitting model in this case was ~85% accurate, which was slightly lower than the random forest in the case study. I did not do much tuning, and only did a few folds, so its possible I may be able to improve it. 
